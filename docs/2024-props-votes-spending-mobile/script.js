...
d3.csv("data.csv").then(function (data) {
    ...
    var xAxis = d3.axisBottom(xScale)
        .tickFormat(function (d) {
            var formattedValue = (d / 1000000).toFixed(1); // Format to nearest million with one decimal place
            return formattedValue.endsWith('.0') ? "$" + formattedValue.slice(0, -2) : "$" + formattedValue; // Remove decimal and trailing zero if it's '.0'
        })
        .ticks(4);
...
svg.append("text")
    ...
    var legendBackground = legend.append("rect")
        ...
        .attr("width", width - 20) // Adjust as needed for width
